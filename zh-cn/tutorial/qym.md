...
和弦符号 `&` 连接两个音高，表示这两个音同时开始演奏，并且演奏相同的时值。如 `1&3` 表示由 do 和 mi 组成的和弦。和弦也可以有多于两个音高，只要在每两个音高之间都写上和弦符号 `&` 即可。和弦的时值（即 `-`、`_` 和 `.`）记录在最后一个音高之后。

例：输入一个大三和弦

	1&3&5---||

> QYS 中，和弦用一对方括号 `[]` 表示,如上例表示为 `[135]---`。

### 快速和弦记号

对于一些常见的和弦（如大三和弦），还可以用快速和弦记号方便地表示。所有快速和弦记号如下表所示：
